...
export class GarantiaComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.garantiaService
      ...
      .subscribe((res: HttpResponse<IGarantia[]>) => this.paginateGarantias(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IGarantia) {
    return item.id;
  }
  ...
  protected paginateGarantias(data: IGarantia[], headers: HttpHeaders) {
