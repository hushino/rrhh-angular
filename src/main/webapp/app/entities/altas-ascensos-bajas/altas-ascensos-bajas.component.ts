...
export class AltasAscensosBajasComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.altasAscensosBajasService
      ...
      .subscribe((res: HttpResponse<IAltasAscensosBajas[]>) => this.paginateAltasAscensosBajas(res.body, res.headers));
  }
  ...
  trackId(index: number, item: IAltasAscensosBajas) {
    return item.id;
  }

  registerChangeInAltasAscensosBajas() {
    this.eventSubscriber = this.eventManager.subscribe('altasAscensosBajasListModification', response => this.loadAll());
  }
  ...
  protected paginateAltasAscensosBajas(data: IAltasAscensosBajas[], headers: HttpHeaders) {
